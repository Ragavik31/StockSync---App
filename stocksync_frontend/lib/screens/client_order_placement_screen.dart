...
class _ClientOrderPlacementScreenState extends State<ClientOrderPlacementScreen> {
  ...
  Future<void> _placeOrder() async {
    ...
    if (paymentMethod == "online") {
      if (kIsWeb) {
        // Razorpay Flutter does not support Web out of the box. Mocking payment for web.
        await Future.delayed(const Duration(seconds: 1));
        if (!mounted) return;
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text("Mock Online Payment Successful on Web")));
      } else {
